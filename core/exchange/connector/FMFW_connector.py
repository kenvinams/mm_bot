...
class FMFWConnector(BaseConnector):
    ...
    def _modify_order_model(self, response):
        ...
        spot_order = SpotOrder(
            ...
            float(response["quantity_cumulative"]),
            created_at,
            updated_at,
        )
        return spot_order
    ...
    async def _get_order_book(self, symbols: List[str]):
        ...
        """
        response = await self._curl(
            "/api/3/public/orderbook", query={"depth": 0, "symbols": ",".join(symbols)}
        )
    ...
    async def _create_spot_order(self, spot_order: SpotOrder):
        ...
        response = await self._curl(
            "/api/3/spot/order", auth=True, verb="POST", post_dict=post_dict
        )
    ...
    async def _cancel_spot_order(self, spot_order: SpotOrder):
        ...
        response = await self._curl(
            "/api/3/spot/order/", auth=True, verb="DELETE", attribute=client_order_id
        )
        if response is not None:
    ...
    async def _get_tickers(self, symbols: List[str]):
        ...
        if len(response) != len(symbols):
            ...
            for s in symbols:
                ...
                ticker = Tickers(
                    unix_time,
                    float(response[s]["open"]),
                    float(response[s]["high"]),
                    float(response[s]["low"]),
                    float(response[s]["last"]),
                    float(response[s]["ask"]),
                    float(response[s]["bid"]),
                    float(response[s]["volume"]),
                )
                tickers[s] = ticker
            return tickers
    ...
    async def _get_commission(self, symbols: List[str]) -> dict:
        ...
        if all(temp):
            return {
                symbols[i]: tuple([temp[i]["take_rate"], temp[i]["make_rate"]])
    ...
    async def _curl(
        ...
        async def retry():
            r = await self._curl(
                path, auth, verb, query, post_dict, attribute, retry_count + 1
            )
            return r
        ...
        try:
            ...
            if verb == "GET":
                ...
                async with self._session.post(
                    url, headers=headers, data=post_dict
                ) as resp:
                    resp.raise_for_status()
                    response = await resp.text()
            elif verb == "PATCH":
